...
Private Const BOOK_KIND As String = "team"
...
Public Sub PythonRefreshViews()
    RunPythonAsync "refresh-views " & BOOK_KIND & " " & QuoteArg(ThisWorkbook.FullName) & " --backup-dir " & _
        QuoteArg(ThisWorkbook.Path & Application.PathSeparator & "backup"), "テーマ一覧更新"
End Sub
...
Public Sub PythonSyncGanttDates()
    RunPythonAsync "sync-gantt-dates " & BOOK_KIND & " " & QuoteArg(ThisWorkbook.FullName) & " --backup-dir " & _
        QuoteArg(ThisWorkbook.Path & Application.PathSeparator & "backup"), "週ガント逆同期"
End Sub
